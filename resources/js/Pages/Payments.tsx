...
export default function Payments({ contracts, interestData, filters }: any) {
  ...
  const renderCellContent = (col: PaymentsColumnDef, row: any): React.ReactNode => {
    ...
    switch (col.id) {
      ...
      case "paid":
        return <span className="mono" style={{ color: "#059669", fontWeight: 600 }}>{interest?.paidInstallments ?? 0}</span>;
      case "overdue":
        return <span className="mono" style={{ color: "#dc2626", fontWeight: 600 }}>{interest?.overdueInstallments ?? 0}</span>;
      case "daysOverdue":
        return (
          <span className="mono" style={{ color: (interest?.maxDaysOverdue ?? 0) > 0 ? "#dc2626" : "inherit" }}>
            {interest?.maxDaysOverdue ? Math.trunc(interest.maxDaysOverdue) : "—"}
          </span>
        );
      case "toReceive":
              ...
              {schedule?.schedule?.map((inst: any) => {
                ...
                return (
                  <tr key={`inst-row-${inst.installmentId}`} style={{ background: rBg }} onMouseOver={e => (e.currentTarget.style.background = "#eff6ff")} onMouseOut={e => (e.currentTarget.style.background = rBg)}>
                    ...
                    <td style={{ textAlign: "center", padding: "5px 8px", fontFamily: "monospace", color: inst.daysOverdue > 0 ? "#dc2626" : "#6b7280", fontWeight: 700, borderBottom: "1px solid #e5e7eb" }}>{inst.daysOverdue > 0 ? Math.trunc(inst.daysOverdue) : "—"}</td>
