...
export default function Lancamentos({ contracts, clients, kpis, filters }: any) {
  ...
  const renderCellContent = (col: LancamentoColumnDef, c: any) => {
    switch (col.id) {
      ...
      case "overdue": {
        const totalInst = Number(c.installmentCount ?? 0);
        const paidCount = Number(c.paidInstallmentsCount ?? 0);
        const openCount = totalInst - paidCount;
        return <span style={{ fontFamily: FONT_MONO, fontWeight: 700, color: "#dc2626" }}>{openCount}</span>;
      }
      case "daysOverdue": {
        const cleanDays = Math.floor(Number(c.maxDaysOverdue ?? 0));
        return (
          <span style={{
            fontFamily: FONT_MONO, fontWeight: 700,
            color: cleanDays > 0 ? "#dc2626" : "#059669",
          }}>
          ...
          <div style={{
            ...
            <div style={{ position:"relative", flex:"1 1 180px", maxWidth: 260, minWidth: 160 }}>
              ...
              <input
                ...
                value={search}
                onChange={e => { setSearch(e.target.value); handleFilterChange(e.target.value, statusFilter); setPage(1); }}
              />
            </div>
            <select
              ...
              value={statusFilter}
              onChange={e => { setStatusFilter(e.target.value); handleFilterChange(search, e.target.value); setPage(1); }}
            >
